...
\tcnote{Compare with: 
\[
\Bigl(\bar{\Pi} -\mathbb{E}(A^TA) \Bigr)^2 =  \Bigl(\bar{\Pi} - \frac{d}{p} I_{p\times p}\Bigr)^2
\]
This gave us the same answer, right?}
...
\KwResult{Output an ordered set of basis vectors with associated weightings}
 ...
 \For{$b_1 = 1,2,\dots,B_1$}{
    ...
    Compute $\Pi = \frac{1}{B_1}\sum\limits_{b_1 = 1}^{B_1}A_{b_1}^TA_{b_1}$\;
...
For a projection $A$, let our risk estimate be given by $\hat{R}_A$, where $\hat{R}_A$ takes values in a set $\mathcal{R}$ (for example we may have for validation error $\mathcal{R} = \{0, \frac{1}{n_{\text{val}}}, \frac{2}{n_{\text{val}}}, \ldots, 1 \} $). We assume that $\mathcal{R}$ is finite\footnote{\jbnote{I don't think finite's necessary but pah}} and has at least two distinct values $r_1, r_2 \in \mathcal{R}$ such that $\mathbb{P}(\hat{R}_A = r_i) \neq 0$. We then define the function\footnote{\jbnote{This is expectation over random A, but I don't know how to bold blackboard bold.}} 
...
\begin{align*}
    ...
    % \mathbb{P}(\hat{R}_1 = \hat{R}_2) & = (1 - \Phi\big(\frac{\Delta}{2}|\cos\theta|\big))(1-\Phi\big(\frac{\Delta}{2}|\sin\theta|\big)) + 
    % \Phi\big(\frac{\Delta}{2}|\cos\theta|\big))\Phi\big(\frac{\Delta}{2}|\sin\theta|\big)
\end{align*}
