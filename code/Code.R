...
## Simulation model 1
# Simulate data
simulated_data_1 <- RPModel(1, n = n_train + n_val + n_test, p = p) # changed to model 0 so that highest-variance dimensions aren't the sufficient dimensions
...
model_0_performance <- data.frame(Model = c("RP-LDA", "RP-QDA", "RP-KNN", "SIR", "SAVE", "PHD", "PCA"), 
                                   ...
                                   False_Discovery = c(rp_lda_fd_1, rp_qda_fd_1, rp_knn_fd_1, sir_fd_1, save_fd_1, phd_fd_1, pca_fd_1),
...
# Simulate data
set.seed(1234)
simulated_data_2 <- RPModel(2, n = n_train + n_val + n_test, p = p) # changed to model 0 so that highest-variance dimensions aren't the sufficient dimensions
...
reduce_sir_2_train <- as.matrix(train_val_data_2 %>% dplyr::select(-Y)) %*% sir_2$evectors[, 1:2] %>% 
  as.data.frame() %>% 
  mutate(Dim_1 = Dir1, Dim_2 = Dir2, Y = c(YTrain_2, YVal_2), Model = 1, method = "SIR") %>% 
  dplyr::select(-c(Dir1, Dir2))

reduce_sir_2_test <- XTest_2 %*% sir_2$evectors[, 1:2] %>% 
  as.data.frame() %>% 
  mutate(Dim_1 = Dir1, Dim_2 = Dir2, Y = YTest_2, Model = 1, method = "SIR") %>% 
  dplyr::select(-c(Dir1, Dir2))
...
reduce_save_2_train <- as.matrix(train_val_data_2 %>% dplyr::select(-Y)) %*% save_2$evectors[, 1:2] %>% 
  as.data.frame() %>% 
  mutate(Dim_1 = Dir1, Dim_2 = Dir2, Y = c(YTrain_2, YVal_2), Model = 1, method = "SAVE") %>% 
  dplyr::select(-c(Dir1, Dir2))

reduce_save_2_test <- XTest_2 %*% save_2$evectors[, 1:2] %>% 
  as.data.frame() %>% 
  mutate(Dim_1 = Dir1, Dim_2 = Dir2, Y = YTest_2, Model = 1, method = "SAVE") %>% 
  dplyr::select(-c(Dir1, Dir2))
...
reduce_phd_2_train <- as.matrix(train_val_data_2 %>% dplyr::select(-Y)) %*% phd_2$evectors[, 1:2] %>% 
  as.data.frame() %>% 
  mutate(Dim_1 = Dir1, Dim_2 = Dir2, Y = c(YTrain_2, YVal_2), Model = 1, method = "PHD") %>% 
  dplyr::select(-c(Dir1, Dir2))

reduce_phd_2_test <- XTest_2 %*% phd_2$evectors[, 1:2] %>% 
  as.data.frame() %>% 
  mutate(Dim_1 = Dir1, Dim_2 = Dir2, Y = YTest_2, Model = 1, method = "PHD") %>% 
  dplyr::select(-c(Dir1, Dir2))

# PCA Method
decompose_pca_2 <- svd(rbind(XTrain_2, XVal_2))$v[, 1:2]
colnames(decompose_pca_2) <- c("Dim_1", "Dim_2")

reduce_pca_2_train <- as.matrix(train_val_data_2 %>% dplyr::select(-Y)) %*% decompose_pca_2 %>% 
  as.data.frame() %>% 
  mutate(Y = c(YTrain_2, YVal_2), Model = 1, method = "PCA")

reduce_pca_2_test <- XTest_2 %*% decompose_pca_2 %>% 
  as.data.frame() %>% 
  mutate(Y = YTest_2, Model = 1, method = "PCA")
...
pca_fd_2 <- sum((diag((diag(100) - true_proj_2) %*% pca_est_proj_2)))
pca_pw_2 <- sum(diag(true_proj_2 %*% pca_est_proj_2))
...
model_2_performance <- data.frame(Model = c("RP-LDA", "RP-QDA", "RP-KNN", "SIR", "SAVE", "PHD", "PCA"), 
                                  ...
                                  False_Discovery = c(rp_lda_fd_2, rp_qda_fd_2, rp_knn_fd_2, sir_fd_2, save_fd_2, phd_fd_2, pca_fd_2),
